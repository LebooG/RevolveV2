...
export default function AddTenantScreen({ navigation }: any) {
  ...
  const handleSubmit = async () => {
    if (!name || !phone || !rent) { Alert.alert('Required', 'Name, phone, and rent are required'); return; }
    setLoading(true);
    try {
      await api.addTenant({
        name, phone: phone.startsWith('254') ? phone : `254${phone.replace(/^0/, '')}`,
        email: email || undefined, unitId: 'unit-placeholder',
        leaseStart: new Date().toISOString(), leaseEnd: new Date(Date.now() + 365 * 86400000).toISOString(),
        rentAmount: parseInt(rent),
      });
      Alert.alert('Success', 'Tenant added', [{ text: 'OK', onPress: () => navigation.goBack() }]);
    } catch (err: any) {
      Alert.alert('Error', err?.response?.data?.message || 'Failed to add tenant');
  ...
  return (
    <SafeAreaView style={styles.c}>
      <View style={styles.h}>
        ...
        <Text style={styles.t}>Add Tenant</Text>
        <View style={{ width: 22 }} />
      </View>
      <View style={styles.form}>
        {[
          { label: 'Full Name', value: name, set: setName, ph: 'e.g. Alex Kipchoge' },
          { label: 'Phone Number', value: phone, set: setPhone, ph: '0712345678', kb: 'phone-pad' as const },
          { label: 'Email (optional)', value: email, set: setEmail, ph: 'alex@email.com', kb: 'email-address' as const },
          { label: 'Monthly Rent (KES)', value: rent, set: setRent, ph: 'e.g. 65000', kb: 'numeric' as const },
        ].map((f, i) => (
        ...
        <TouchableOpacity style={[styles.btn, loading && { opacity: 0.6 }]} onPress={handleSubmit} disabled={loading}>
          <Text style={styles.btnText}>{loading ? 'Adding...' : 'Add Tenant'}</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
